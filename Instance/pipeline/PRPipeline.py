...
class PRPipeline:
    ...
    async def arun(self, query: Query):
        ...
        t = time()
        # query = await self.structureMethod.aprocess(query)
        query = await self.structureMethod.aprocess(query)
        res_dict["structureMethodRetrievalModuleTime"] = time()-t

        t = time()
        query = await self.semanticMethod.aprocess(query)
        ...
        res_dict["llm_call"] = query.llm_call
