...
from utils.Tools import get_query, Tools, get_query_subgraph
...
class QueryProcessor:
    def __init__(self, dataset, ppr_file, ModuleLLM):
        ...
        self.ModulePPR = "PreRetrievalModulePPR(max_ent=2000)"
...
if __name__ == '__main__':
    ...
    ave_tokens = {
        "qwen2-70b": {"edge": 5.7, "node": 8.8, "triple": 25.4},
        # "llama3-70b": {"edge": 5.7, "node": 6.8, "triple": 21.4},
    }
    rank_models = {
        "BM25": BM25Model(),
        # "EMB": EmbeddingModel(),
    ...
    if True:
        for scale in scales:
            for model, token_info in ave_tokens.items():
                for reasoning_dataset in ["webqsp", "CWQ", "GrailQA", "WebQuestion"]:
                    ppr_file = f"/back-up/gzy/dataset/VLDB/SE/{reasoning_dataset}/subgraph/PPR.json"
                    ...
                    for rankModelName, rankModel in rank_models.items():
                        ...
                        for task, ModuleLLM in ModuleLLMs.items():
                            print("Task: ", task)
                            window = int(
                                scale*(token_info["triple"]/token_info[task]))
                            ...
                            end_time = time.time()
                            json_path = f"/back-up/gzy/dataset/VLDB/SE/{reasoning_dataset}/subgraph/LLM_scale/{model}/{rankModelName}/{task}/{scale}.json"
