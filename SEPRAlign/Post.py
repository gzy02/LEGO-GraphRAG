...
postRetrievalPipeline = [
    ...
    #  "/back-up/gzy/dataset/VLDB/Pipeline/PathRetrieval/{dataset}/PPR/SPR/BGE.json", PostRetrievalModuleLLM(tokenizer)),
    ("SPR/LLM/qwen2-70b/EMB_v2",
     "/back-up/gzy/dataset/VLDB/Pipeline/PathRetrieval/{dataset}/{subgraph_type}/SPR/EMB.json", PostRetrievalModuleLLM(tokenizer))
]
    ...
    for reasoning_dataset in dataset_list:
        for subgraph_type in subgraph_list:
            for retrievaltype, json_path, module in postRetrievalPipeline:
                ...
                for query in tqdm(get_query_subgraph(test_file), total=1000):
                    if query.qid not in id2paths:
                        continue
