...
class RetrievalModuleBFS(RetrievalModule):
    ...
    def get_reasoning_paths(self, G: ig.Graph, entities: List[str]) -> List[ReasoningPath]:
        ...
        for entity in entities:
            ...
            #    in_paths.update(map(tuple, G.get_all_simple_paths(
            #        v=source, cutoff=hop, mode="in")))
            # for path in in_paths:
            #    reasoning_path = ReasoningPath(
            #        G.vs[path[-1]]['name'])  # 注意起点是path[-1]，而不是path[0]
            #    for i in range(len(path) - 2, -1, -1):  # 注意倒序建立路径
            #        edge_id = G.get_eid(path[i+1], path[i])
            #        edge = G.es[edge_id]
            #        triple = (G.vs[path[i+1]]['name'],
            #                  edge['name'], G.vs[path[i]]['name'])
            #        reasoning_path.add_triple(triple)
            #    reasoning_paths.append(reasoning_path)

            out_paths = G.get_all_simple_paths(
                v=source, cutoff=self.hop, mode="out")
            # G.get_all_simple_paths(
            #    v=source, cutoff=self.hop, mode="out")
