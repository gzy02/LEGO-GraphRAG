...
public:
    ...
    PPR(string name, double e_rate, double e_eps, int k_num, double input_alpha) {
        ...
        string filename = "dataset/Graph.txt";
    ...
    double* MonteCarlo(int u, double walk_num){
        ...
        for(int i =0;i<k;i++){
            cout << "11node: " << newLeftVec[i] << " " << resultList[newLeftVec[i]] << " " << realMap[newLeftVec[i]] << endl;
        }
    ...
    vector<pair<int, double> > TopPPR(int s, double r_threshold, int k){
        ...
        while(true){   
            ...
            if(candidate_size <= backSizeThreshold || leftCandidateSize >= k - error_num || gap2 < 0.0000000000001/*|| (r_sum > 0 && r_sum < 0.0000000001)*/){
                clock_t t3_1 = clock();
                ...
                // fileNameStream <<"Topppr-answer/"<<s<<"/"<< "_output.txt"; // 生成文件名  

                // std::string fileName = fileNameStream.str(); // 获取字符串  

                // // 创建输出文件流  
                // // std::ofstream outFile(fileName);  
                // outFile.open(fileName); 
                    // 指定文件夹路径  
                std::string folderPath = "Topppr-answer/";  
                // 指定文件路径  
                std::string filePath = folderPath +target_filename+"_output.txt";  
                ...
                std::unordered_set<int> writtenSet; // 用于存储已经写入的值
                int incount = 0;
                for (int i = 0; i < newLeftVec.size(); i++) {
                    if (writtenSet.find(newLeftVec[i]) == writtenSet.end()) { 
                        // 如果 newLeftVec[i] 不在集合中
                        outFile << newLeftVec[i] << " " << realMap[newLeftVec[i]] << std::endl; // 写入文件
                        writtenSet.insert(newLeftVec[i]); // 将 newLeftVec[i] 添加到集合中
                        incount++;
                    } else {
                        // 已经写入过的值，跳过
                        // 如果需要调试，可以打印：
                        // std::cout << "跳过重复值: " << newLeftVec[i] << std::endl;
                    }
                    if(incount >=2000){
                        break;
                    }
