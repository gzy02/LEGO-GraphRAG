...
int main(int argc, char *argv[]){
    ...
    if(algo == "GEN_QUERY"){
        ...
        if(!ppr.is_file_exist(queryname)){
            ...
            ppr.PowerMethodMulti(100, node_count, 16);/*  多线程PowerMethparameter: iteration loops, node size, thread num */
    }
    else{
        string queryname = "dataset/" + filename + ".query";
        if(!ppr.is_file_exist(queryname)){
        ...
        for(int para = 0; para < 1; para++){
            ...
            for(int t = 0; t < node_count; t++){
                ...
                outfile.open("TopPPR_topk.txt", std::ios_base::app);  // 以追加模式打开文件
                if (outfile.is_open()) {
                    outfile << filename <<" "<< duration.count() << "\n";
                    outfile.close();  // 关闭文件
                } else {
            ...
            double result_time = ppr.avg_time;
            cout << "avg time: " << result_time / (double) node_count << endl;
