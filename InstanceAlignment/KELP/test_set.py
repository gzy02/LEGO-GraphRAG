...
#    os.environ["OPENAI_API_KEY"] = f.read().split("=")[-1].strip()
openai.api_key = ""
...
if hop == 1:
    ...
    save_file = f'data/{dataset}/test_onehop.jsonl'
if hop == 2:
    question_data = f'data/{dataset}/train.jsonl'
    save_file = f'data/{dataset}/train_twohop.jsonl'
...
model_name = 'gpt-4o-mini-2024-07-18'  # "gpt-3.5-turbo-0125"  #
model_path = {
...
def llm(prompt, max_tokens=max_tokens):
    output_tokens = max_contexts-get_tokens(prompt)
    if output_tokens < 0:
...
def build_subgraph_rels_ranking(question, entity_set, knowledge_graph):
    one_hop_neighbors = get_one_hop_neighbors_rels_ranking(
        question, entity_set, knowledge_graph)
    if one_hop_neighbors == "No":
        return "No"
    if hop == 1:
        return one_hop_neighbors  # +two_hop_neighbors
    if hop == 2:
        two_hop_neighbors = get_two_hop_neighbors_rels_ranking(
            question, one_hop_neighbors, knowledge_graph)
...
def get_one_hop_neighbors_rels_ranking(question, entity_set, knowledge_graph):
    ...
    top_neighbors_rel = get_top_related_triplets_rels_ranking(
        question, neighbors_rel, 1)
        ...
        if top_neighbors_rel[i][0] in knowledge_graph and top_neighbors_rel[i][1] in knowledge_graph[top_neighbors_rel[i][0]]:
            for obj in knowledge_graph[top_neighbors_rel[i][0]][top_neighbors_rel[i][1]]:
                if (str(top_neighbors_rel[i][0]), str(top_neighbors_rel[i][1]), str(obj)) not in neighbors_ent[i] and (',' not in str(entity) and (',' not in str(relation))) and (',' not in str(obj)):
                    neighbors_ent[i].add(
                        (str(top_neighbors_rel[i][0]), str(top_neighbors_rel[i][1]), str(obj)))
                    num_ent += 1
    top_neighbors_ent = get_top_related_triplets_ents_ranking(
        question, neighbors_ent, 1, num_ent)
...
def get_two_hop_neighbors_rels_ranking(question, top_1hop_triplets, knowledge_graph):
    ...
    for i in range(len(top_neighbors_ent)):
        for triplet in top_1hop_triplets:
            ...
            if t == top_neighbors_ent[i][0]:
                top_neighbors_ent[i] = (str(h), str(r), str(top_neighbors_ent[i][0]), str(
                    top_neighbors_ent[i][1]), str(top_neighbors_ent[i][2]))
                break
    if top_1hop_triplets:
        ...
        if h_n in knowledge_graph:
            r_n = random.choice(list(knowledge_graph[h_n]))
            t_n = random.choice(list(knowledge_graph[h_n][r_n]))
            top_neighbors_ent.append(
                (str(h0), str(r0), str(h_n), str(r_n), str(t_n)))
...
def get_top_related_triplets_rels_ranking(question, triplets, hop):
    ...
    if response is None:
        return "No"
    try:
        ranked_triplets = [tuple(word.strip('\'" ') for word in triplet.split(
            '(')[1].split(')')[0].split(',')) for triplet in response.split(';') if triplet]
    except Exception as e:
        return "No"
    return ranked_triplets
    ...
    while (num_sel < 5 and i < len(neighbors_ent)):
        if len(neighbors_ent[i]) == 1:
            ...
            if response is None:
                return "No"
            try:
                add_info = [tuple(word.strip('\'" ') for word in triplet.split('(')[1].split(')')[
                                  0].split(',')) for triplet in response.split(';') if triplet]
            except Exception as e:
                return "No"
...
original_prompt = open_file(
    './meta_'+str(hop)+'hop_prompts/verify_claim_no_evidence.txt')
context_prompt = open_file(
    './meta_'+str(hop)+'hop_prompts/verify_claim_with_evidence.txt')
...
def context_query(question, ground_truth, triplet, already_pos, already_neg, pos_sam, neg_sam):
    ...
    context += ']'
    prompt = context_prompt.replace('<<<<CLAIM>>>>', question).replace(
...
def process_question(ii):
    try:
        ...
        random.shuffle(tl)
        for i in tl:
            context_correct, already_pos, already_neg, pos_sam, neg_sam = context_query(
                question, ground_truth, subgraph[i], already_pos, already_neg, pos_sam, neg_sam)
...
if __name__ == "__main__":
    # 只处理1/5的questions_dict
    keys = list(questions_dict.keys())
    # subset_keys = keys[:len(keys) // 5]
    subset_keys = keys
    t = time.time()
    with Pool(16) as pool:
        pool.starmap(process_question, [(ii,) for ii in subset_keys])
    print(time.time()-t, 's')
