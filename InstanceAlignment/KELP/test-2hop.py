...
question_model.load_state_dict(torch.load('model/question_model_epoch59.pth'))
...
def open_file(filepath):
    with open(filepath, 'r', encoding='utf-8') as infile:
        return infile.read().split('=')[1].strip()
...
if hop == 2:
    question_data = f'data/{dataset}/test.jsonl'
    note = 'note.txt'
    error_file = f'{dataset}-two-hop-errors.json'
...
def build_subgraph1(entity_set, knowledge_graph):
    subgraph = set()
        ...
        if (subgraph[index][1], subgraph[index][2]) not in already_got_list:
            ...
            for i in range(len(already_got_list)):
                if (len(subgraph[index]) == 5) and (already_got_list[i] == (subgraph[index][1], subgraph[index][2])) and (already_got[i] < k2-1):
                    already_got[i] += 1
                    triplets.append(
                        (subgraph[index][2], subgraph[index][3], subgraph[index][4]))
                    values.append(value)
                    if subgraph[index][3][0] == '~':
                        context_texts += '['+subgraph[index][4]+', ' + \
                            subgraph[index][3][1:]+', ' + \
