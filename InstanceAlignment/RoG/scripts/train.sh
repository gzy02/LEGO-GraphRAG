MODEL_PATH=/back-up/LLMs/llama3/Meta-Llama-3-8B-Instruct/
#DATASET_LIST="../datasets/joint_training/align/cwq/cwq_train.jsonl ../datasets/joint_training/align/webqsp/webqsp_train.jsonl ../datasets/joint_training/qa/webqsp/webqsp_train.jsonl ../datasets/joint_training/qa/cwq/cwq_train.jsonl ../datasets/joint_training/ExplainQAData/cwq/cwq_train_1000.jsonl ../datasets/joint_training/ExplainQAData/webqsp/webqsp_train_1000.jsonl"
DATASET_LIST="../datasets/joint_training/align/cwq/cwq_train.jsonl"
SAVE_NAME=RoG-llama3-8B-Paths
SAVE_PATH=/back-up/gzy/models/RoG-llama3-8B-Paths
ADD_REL=False

accelerate launch --config_file ../config/deepspeed_zero3.yml joint_training/joint_finetuning.py \
    --data_path_list ${DATASET_LIST}  \
    --model_name_or_path ${MODEL_PATH} \
    --output_dir ${SAVE_PATH} \
    --add_rel_token ${ADD_REL} \
    --bf16 True \
    --num_train_epochs 1 \
    --per_device_train_batch_size 4 \
    --per_device_eval_batch_size 4 \
    --gradient_accumulation_steps 16 \
    --evaluation_strategy "yes" \
    --save_strategy "yes" \
    --save_steps 100 \
    --save_total_limit 2 \
    --learning_rate 2e-5 \
    --weight_decay 0. \
    --warmup_ratio 0.03 \
    --lr_scheduler_type "cosine" \
    --logging_steps 20 \
    --tf32 True \
    --gradient_checkpointing True \
    --run_name ${SAVE_NAME}
#--report_to "wandb" \
