...
def formatting_prompts_func(example):
    ...
    graph = build_graph(example["subgraph"])
    paths = get_truth_paths(example["entities"], example["answers"], graph)
    ground_paths = set()
    for path in paths:
    ...
    if prediction is not None:
        output_text = "[INST] <<SYS>>\n<</SYS>>\n" + input_text + \
            ' [/INST] ' + prediction + tokenizer.eos_token
...
for data_name in data_list:
    ...
    save_path = os.path.join(
        save_dir, data_name, data_name + "_train_1000.jsonl")
