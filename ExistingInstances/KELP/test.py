...
with open('data/freebase_kg.pickle', 'rb') as f:
    kg = pickle.load(f)

dataset = "webqsp"
question_model.load_state_dict(torch.load(
    f'models/{dataset}/2-hop/question_model_epoch59.pth'))

questions_dict = {}
entity_set_dict = {}
label_set_dict = {}

question_data = f'data/{dataset}/test.jsonl'
note = f'note_{dataset}.txt'
error_file = f'{dataset}-one-hop-errors.json'
...
def build_subgraph2(entity_set, knowledge_graph):
    subgraph = set()
    for entity in entity_set:
        if entity in knowledge_graph:
            for relation, obj in knowledge_graph[entity].items():
                for obj in knowledge_graph[entity][relation]:
                    subgraph.add((str(entity), str(relation), str(obj)))
                    if obj in knowledge_graph:
                        for relation2, obj2 in knowledge_graph[obj].items():
                            for obj2 in knowledge_graph[obj][relation2]:
                                subgraph.add((str(entity), str(relation), str(
                                    obj), str(relation2), str(obj2)))
...
def context_query(question_list, ground_truth_list, context_texts):
    ...
    for _ in range(3):
        ...
        prompt += 'Now answer the following ' + \
            ...
            ' questions in the same way of these examples.\n'
        j = 0
        for question in question_list:
            j += 1
            prompt += 'Context ' + \
                str(j)+f': {context_texts[0]}' + \
                f' Question '+str(j)+f': {question}'+'\n'
        prompt += 'Answer '+str(j)+': '
        result = llm(prompt)
...
def process_question(ii):
    global question_id_list, question_list, entity_set_list, ground_truth_list, contexts_list, total_correct, dataset_len, a, k1, k2, question_data, note, error_file, questions_dict, entity_set_dict, label_set_dict, kg, device, question_model, tokenizer, criterion
    ...
    question_input = tokenizer(
        [question], return_tensors="pt", padding=True, truncation=True).to(device)
    question_embedding = question_model(
        **question_input).last_hidden_state.mean(dim=1)

    def format_triplet(triplet):
        if len(triplet) == 3:
            if triplet[1][0] == '~':
                return f"{triplet[2]} {triplet[1][1:]} {triplet[0]}."
            else:
                return f"{triplet[0]} {triplet[1]} {triplet[2]}."
        elif len(triplet) == 5:
            if triplet[1][0] == '~':
                pos = f"{triplet[2]} {triplet[1][1:]} {triplet[0]}, "
            else:
                pos = f"{triplet[0]} {triplet[1]} {triplet[2]}, "
            if triplet[3][0] == '~':
                pos += f"{triplet[4]} {triplet[3][1:]} {triplet[2]}."
            else:
                pos += f"{triplet[2]} {triplet[3]} {triplet[4]}."
            return pos

    def process_pos_list(pos_list):
        positive_input = tokenizer(
            pos_list, return_tensors="pt", padding=True, truncation=True).to(device)
        positive_embedding = question_model(
            **positive_input).last_hidden_state.mean(dim=1)
        similarity_scores_pos = criterion(
            question_embedding, positive_embedding).tolist()
        return similarity_scores_pos

    batch_size = 128
    pos_list = [format_triplet(triplet) for triplet in subgraph]
    # print('len(pos_list):', len(pos_list))
    # 分批处理 pos_list
    for i in range(0, len(pos_list), batch_size):
        batch = pos_list[i:i + batch_size]
        cossim += process_pos_list(batch)
    ...
    for index, value in sorted_lst:
        ...
        if len(subgraph[index]) == 3:
            if subgraph[index][1][0] == '~':
                context_texts += '['+subgraph[index][2]+', ' + \
                    subgraph[index][1][1:]+', '+subgraph[index][0]+'], '
            else:
                context_texts += '['+subgraph[index][0]+', ' + \
                    subgraph[index][1]+', '+subgraph[index][2]+'], '
            ...
            if subgraph[index][3][0] == '~':
                context_texts += '['+subgraph[index][4]+', ' + \
                    subgraph[index][3][1:]+', '+subgraph[index][2]+'], '
            else:
                context_texts += '['+subgraph[index][2]+', ' + \
                    subgraph[index][3]+', '+subgraph[index][4]+'], '
    ...
    if len(question_id_list) >= 1:
        cor_list = context_query(
            question_list, ground_truth_list, contexts_list)
        if cor_list == "answer length error":
            ...
            with open(note, 'w') as file:
                file.write(str(total_correct)+'/'+str(dataset_len)+'    ')
                file.write(str(total_correct/dataset_len))
                file.write('\n')
            question_id_list = []
            question_list = []
            entity_set_list = []
...
index = 0
for ii in tqdm(questions_dict):
    index += 1
    process_question(ii)

# 使用ThreadPoolExecutor进行多线程处理
# with concurrent.futures.ThreadPoolExecutor() as executor:
#    list(tqdm(executor.map(process_question, questions_dict), total=len(questions_dict)))
...
print('Acc: ', total_correct, '/', dataset_len, '=', total_correct/dataset_len)
