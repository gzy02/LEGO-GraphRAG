...
def main(args, LLM):
    ...
    output_dir = os.path.join(
        args.predict_path, args.reason_type, args.model_name, args.split, rule_postfix
    )
...
if __name__ == "__main__":
    ...
    prompt_path = "../prompts/llama2_predict.txt"  # "../prompts/general_prompt.txt"
    argparser = argparse.ArgumentParser()
    argparser.add_argument(
        "--data_path", type=str, default=f"/back-up/gzy/dataset/AAAI/MultiHopExperiment/{reasoning_dataset}/{reasoning_type}/"
    )
    argparser.add_argument("--d", "-d", type=str, default="test_name.jsonl")
    ...
    argparser.add_argument("--reason_type", type=str,
    ...
    argparser.add_argument("--add_rule", action="store_true", default=True)
    argparser.add_argument("--use_true", action="store_true", default=False)
    argparser.add_argument("--cot", action="store_true")
    argparser.add_argument("--explain", action="store_true")
    argparser.add_argument("--use_random", action="store_true", default=False)
    argparser.add_argument("--each_line", action="store_true")
    argparser.add_argument(
        "--rule_path",
        type=str,
        default="results/gen_rule_path/" +
        f"{reasoning_dataset}-{reasoning_type}" +
        "/RoG/test/predictions_3_False.jsonl",
    )
    argparser.add_argument(
