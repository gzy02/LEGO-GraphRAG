    ...
    with open(predict_file, 'r') as f, open(detailed_eval_file, 'w') as f2:
        for line in f:
            ...
            if cal_f1:
                ...
                acc_list.append(acc)
                hit_list.append(hit)
                f2.write(json.dumps({'id': id, 'prediction': prediction, 'ground_truth': answer, 'acc': acc,
                         'hit': hit, 'f1': f1_score, 'precission': precision_score, 'recall': recall_score}) + '\n')
            else:
                acc = eval_acc(prediction, answer)
                hit = eval_hit(prediction, answer)
                acc_list.append(acc)
                hit_list.append(hit)
                f2.write(json.dumps({'id': id, 'prediction': prediction,
                         'ground_truth': answer, 'acc': acc, 'hit': hit}) + '\n')

    if len(f1_list) > 0:
        result_str = "Accuracy: " + str(sum(acc_list) * 100 / len(acc_list)) + " Hit: " + str(sum(hit_list) * 100 / len(hit_list)) + " F1: " + str(sum(f1_list) * 100 / len(
            f1_list)) + " Precision: " + str(sum(precission_list) * 100 / len(precission_list)) + " Recall: " + str(sum(recall_list) * 100 / len(recall_list))
    else:
        result_str = "Accuracy: " + \
            str(sum(acc_list) * 100 / len(acc_list)) + " Hit: " + \
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        '-d', type=str, default='results/KGQA/CWQ-PPR2/Llama3/test/results_gen_rule_path_CWQ-PPR2_RoG_test_predictions_3_False_jsonl/predictions.jsonl')
    argparser.add_argument('--cal_f1', action="store_true", default=False)
