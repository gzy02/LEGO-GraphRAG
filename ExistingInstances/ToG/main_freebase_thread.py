...
def process_data(index, data, question_string, args):
    try:
            ...
            for entity in current_entity_relations_list:
                if entity['head']:
                    entity_candidates_id = entity_search(
                        entity['entity'], entity['relation'], True)
                else:
                    entity_candidates_id = entity_search(
                        entity['entity'], entity['relation'], False)

                if args.prune_tools == "llm":
                    if len(entity_candidates_id) >= 20:
                        entity_candidates_id = random.sample(
                            entity_candidates_id, args.num_retain_entity)
                ...
                total_candidates, total_scores, total_relations, total_entities_id, total_topic_entities, total_head = update_history(
                    entity_candidates, entity, scores, entity_candidates_id, total_candidates, total_scores, total_relations, total_entities_id, total_topic_entities, total_head)
                ...
                if stop:
                    ...
                    if flag_finish:
                        ...
                        topic_entity = {entity: id2entity_name_or_type(
                            entity) for entity in entities_id}
                        continue
            else:
                half_stop(question, cluster_chain_of_entities, depth, args)
                flag_printed = True
        ...
        print("Error in question %d." % index)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--LLM_name", type=str,
                        default="llama2-7b", help="base LLM model.")  # meta-llama/
    parser.add_argument("--LLM_type", type=str,
                        default="llama2-7b", help="base LLM model.")
    ...
    with Pool(cpu_count()*4) as pool:
        pool.starmap(process_data, [
                     (index, datas[index], question_string, args) for index in range(len(datas))])
