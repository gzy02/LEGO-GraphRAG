...
void fora_query_topk_with_bound(int v, const Graph& graph){
    // save top k query to txt in this function
    Timer timer(0);
    const static double min_delta = 1.0 / graph.n;
...
//     int idx = fwd_idx.first.occur[i];
//     double value = fwd_idx.first.m_data[idx];
//     std::cout << "First map - Index: " << idx << ", Value: " << value << std::endl;
// }
// for (int i = 0; i < fwd_idx.second.occur.m_num; ++i) {
//     int idx = fwd_idx.second.occur[i];
//     double value = fwd_idx.second.m_data[idx];
//     std::cout << "Second map - Index: " << idx << ", Value: " << value << std::endl;
// }
    // Top-K 查询结果存储到 txt 文件中
    // std::vector<std::pair<int, double>> topk_results;
    // for (auto& p : fwd_idx.first) { // 遍历 reserve 集合
    //     topk_results.emplace_back(p.first, p.second);
    // }

    // 按 PPR 值从高到低排序
    // std::sort(topk_results.begin(), topk_results.end(),
    //           [](const std::pair<int, double>& a, const std::pair<int, double>& b) {
    //               return a.second > b.second;
    //           });

    // // 打开文件并写入
    // std::ofstream outfile("topk_results.txt");
    // if (outfile.is_open()) {
    //     for (size_t i = 0; i < std::min(topk_results.size(), static_cast<size_t>(3000)); ++i) { // 假设 k 是 Top-K 的参数
    //         outfile << "Node: " << topk_results[i].first
    //                 << ", PPR Score: " << topk_results[i].second << "\n";
    //     }
    //     outfile.close();
    // } else {
...
void get_topk(int v, Graph &graph){
    ...
    outfile.open("foraplus_topk.txt", std::ios_base::app);  // 以追加模式打开文件
    if (outfile.is_open()) {
        outfile << config.graph_alias <<" "<< duration.count() << " seconds\n";
        outfile.close();  // 关闭文件
    } else {
...
void topk(Graph& graph){
    ...
    // }
        // 打开文件输出
    std::ofstream outfile("foraresultplus/"+config.graph_alias+"_output.txt");
    // if (!outfile.is_open()) {
