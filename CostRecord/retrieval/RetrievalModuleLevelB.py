...
class RetrievalModuleSemanticModel(RetrievalModule):
    ...
    def process(self, query: Query) -> Query:
        query.reasoning_paths, st_tokens = self.get_reasoning_paths(
            query.subgraph, query.entities, query.question)
        query.st_tokens += st_tokens
        return query
    ...
    def get_reasoning_paths(self, G: ig.Graph, entities: List[str], question: str) -> List[ReasoningPath]:
        ...
        for entity in entities:
            ...
            for step in range(self.hop):
                ...
                st_token = self.model.token_count(
                    question)+sum([self.model.token_count(path) for path in corpus])
                st_tokens += st_token
        ...
        return reasoning_paths, st_tokens
    ...
    def get_reasoning_paths(self, G: ig.Graph, entities: List[str], question: str) -> List[ReasoningPath]:
        ...
        for entity in entities:
            ...
            while queue:
                ...
                new_paths, st_token = self.semantic_extract(
                    G, next_queue, question)
                st_tokens += st_token
